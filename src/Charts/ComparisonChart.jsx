...
const ComparisonChart = () => {
  ...
  // Create color scale
  const colorScale = d3.scaleOrdinal().domain(demographicOptions).range(colors);
        ...
        <div className="control-group">
          ...
          <div className="demographic-toggles">
            {demographicOptions.map((demo) => (
              <button
                key={demo}
                className={`demographic-toggle ${
                  selectedDemographics.includes(demo) ? "active" : ""
                }`}
      ...
      >
        {error ? (
          ...
          <div className="no-selection-message">
            Please select at least one demographic to compare
          </div>
        ) : (
          <svg
            ref={svgRef}
        ...
        {data && selectedDemographics.length > 0 && (
          <ul className="insights-list">
            {selectedDemographics.map((demo) => {
              const growthData = calculateGrowthData().find(
                (d) => d.demographic === demo
              );
              ...
              return (
                <li
                  key={demo}
                  style={{ borderLeft: `3px solid ${colorScale(demo)}` }}
                >
                  <strong>
                    {demo.charAt(0).toUpperCase() + demo.slice(1)}
                  </strong>
                  :
                  {metric === "total" &&
                    ` ${growthData.endValue.toLocaleString()} degrees in ${
                      yearRange[1]
                    }`}
                  {metric === "growth" &&
                    ` Increased by ${growthData.growth.toLocaleString()} degrees`}
                  {metric === "percentage" &&
                    ` ${growthData.percentage.toFixed(1)}% growth from ${
                      yearRange[0]
                    } to ${yearRange[1]}`}
                </li>
              );
